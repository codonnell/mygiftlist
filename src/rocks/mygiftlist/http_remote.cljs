(ns rocks.mygiftlist.http-remote
  ...
  (:require
   ...
   [com.fulcrologic.fulcro.networking.http-remote :as fhttp]
     ...
     (fn [{:keys [headers body] :as request}]
       (go
         (let [access-token (<! (auth/get-access-token))
               [body response-type] (fhttp/desired-response-type request)
...
(defn fulcro-http-remote
  ...
  [{:keys [url request-middleware response-middleware] :or {url                 "/api"
                                                            response-middleware (fhttp/wrap-fulcro-response)
                        ...
                        (go (let [edn              (eql/ast->query ast)
                                  ...
                                  progress-handler (fn [update-msg]
                                                     (let [msg {:status-code      200
                                                                ...
                                                                :overall-progress (fhttp/progress% update-msg :overall)
                                                                :receive-progress (fhttp/progress% update-msg :receiving)
                                                                :send-progress    (fhttp/progress% update-msg :sending)}]
                                                       (when update-handler
                                                         (try
                                                           (update-handler msg)
                                                           (catch :default e
                              ...
                              (let-chan [real-request (try
                                ...
                                (if real-request
                                  (let [abort-id             (or
                                        ...
                                        xhrio                (fhttp/make-xhrio)
                                        {:keys [body headers url method response-type]} real-request
                                        http-verb            (-> (or method :post) name str/upper-case)
                                        extract-response     #(fhttp/extract-response body real-request xhrio)
                                        extract-response-mw  (fhttp/response-extractor* response-middleware edn real-request xhrio)
                                        gc-network-resources (fhttp/cleanup-routine* abort-id active-requests xhrio)
                                        progress-routine     (fhttp/progress-routine* extract-response progress-handler)
                                        ok-routine           (fhttp/ok-routine* progress-routine extract-response-mw ok-handler error-handler)
                                        error-routine        (fhttp/error-routine* extract-response-mw ok-routine progress-routine error-handler)
                                        with-cleanup         (fn [f] (fn [evt] (try (f evt) (finally (gc-network-resources)))))]
                                    (when abort-id
                                      (swap! active-requests update abort-id (fnil conj #{}) xhrio))
                                    (when (and (fhttp/legal-response-types response-type) (not= :default response-type))
                                      (.setResponseType ^js xhrio (get fhttp/response-types response-type)))
                                    (when progress-handler
                                      (fhttp/xhrio-enable-progress-events xhrio)
                                    ...
                                    (fhttp/xhrio-send xhrio url http-verb body headers))
